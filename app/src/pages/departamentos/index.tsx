...
const Departamentos: React.FC = (props) => {
  const [isLoading, setIsLoading] = useState(true);

  const [TextoBusca, setTextoBusca] = useState<string>('');
  const [Departamentos, setDepartamentos] = useState<Array<object>>([]);
  ...
  const carregarDados = async (TextoBuscaParm = TextoBusca, PaginationModelParm = PaginationModel) => {
    ...
    const Resp = await getDepartamentos(TextoBuscaParm, PaginationModelParm);

    if (Resp) {
      setDepartamentos(Resp.dados);
  ...
  return (
    <ContainerPainel pagina={'Departamentos'}>
      <BoxPrincipal>
        <BoxSuperior>
          <BarraPesquisar filtrarGrid={filtrarGrid} />
          <BotaoAdicionar pagina={ROUTES.DEPARTAMENTOS} />
        </BoxSuperior>
        <BoxGrid>
          <Grid
